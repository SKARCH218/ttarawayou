...
(() => {
  ...
  const state = { days: 2, people: 1, budget: 300000, balance: 0, maxBudget: 300000 };

  // 칩 그룹: data-single 있으면 단일 선택, 없으면 다중 토글
  ...
  const chipValue = (id) => document.querySelector(`#${id} .chip.on`)?.dataset.v || null;
  const chipValues = (id) => [...document.querySelectorAll(`#${id} .chip.on`)].map((c) => c.dataset.v);
  ...
  createBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // 지역을 고르면 GPS 불필요 — 현재 위치 주변일 때만 위치 요청
      const loc = region ? null : await getLocation();
      const res = await fetch(`${API_BASE}/api/plan`, {
        ...
        body: JSON.stringify({
          ...
          region,
          purpose: chipValue('purposeChips'),
          foodPreference: chipValue('foodChips'),
          keywords: chipValues('keywordChips'),
          avoidWalking: document.getElementById('avoidWalking').checked,
          preferenceNote: document.getElementById('prefNote').value.trim() || null,
          gender: document.getElementById('genderSelect').value || null,
          ageGroup: document.getElementById('ageSelect').value || null,
          mbti: document.getElementById('mbtiSelect').value || null,
        }),
      });
      if (!res.ok) {
      ...
      savePlan(plan);
      sessionStorage.removeItem('trevitDoneDay'); // 새 플랜 → 일차 진행 초기화
