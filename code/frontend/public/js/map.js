...
(function init() {
  ...
  const doneDayGate = Number(sessionStorage.getItem('trevitDoneDay') || 0);
  ...
  function htmlMarker(lat, lng, html, w, h) {
    return L.marker([lat, lng], {
      icon: L.divIcon({ html, className: '', iconSize: [w, h], iconAnchor: [w / 2, h / 2] }),
      interactive: false,
    }).addTo(map);
  }

  /** 폴리라인 래퍼 (setLatLngs로 갱신) */
  ...
  const userMarker = {
    ...
    setLatLng(ll) { this.pos = [ll[0], ll[1]]; userMarkerObj.setLatLng([ll[0], ll[1]]); },
    getLatLng() { return { lat: this.pos[0], lng: this.pos[1] }; },
  };

  const revealedPath = makeLine({ color: '#703fe4', weight: 7, opacity: 0.95 });
  const walkedPath = makeLine({ color: '#a7b6b9', weight: 5, opacity: 0.7, dash: true });
    ...
    if (l && l.mode === 'TRANSIT') {
      ...
      if (Array.isArray(l.steps) && l.steps.length) {
        ...
        body = `<b>${l.boardStop || '근처 정류장'}</b>에서 승차 → `
          ...
          + `<span style="color:#6c7e84;">${l.summary || ''}</span>`;
      }
  ...
  function updateLegMarkers() {
    ...
    if (target) mk(target.latitude, target.longitude, badge('?', '목적지', '#703fe4'));
    ...
    if (l && l.mode === 'TRANSIT') {
      ...
      if (board) mk(board[0], board[1], badge('', '승차', '#00b57f'));
      if (alight) mk(alight[0], alight[1], badge('', '하차', '#c90e2e'));
    }
  }
  ...
  $('revealNext').addEventListener('click', () => {
    ...
    if (currentLeg >= legs.length) {
      ...
      const doneDay = Math.max(day, Number(sessionStorage.getItem('trevitDoneDay') || 0));
      sessionStorage.setItem('trevitDoneDay', String(doneDay));
  ...
  function startGps() {
    if (!navigator.geolocation) {
      showDemoFallback('이 기기는 GPS를 지원하지 않아요');
      return;
    }
    navigator.geolocation.watchPosition(
      ...
      (err) => {
        if (err.code === err.PERMISSION_DENIED) {
          showDemoFallback('위치 권한이 없어요');
        } else {
          $('statusSub').textContent = 'GPS 신호를 찾는 중… (실외에서 더 잘 잡혀요)';
        }
      },
  ...
  /** GPS를 못 쓰는 환경(데스크톱 심사 등) → 데모 모드 안내 */
  function showDemoFallback(reason) {
    $('statusSub').innerHTML = `${reason} — `
      + `<a href="map.html?day=${day}&demo=1" style="color:#b39bf0; font-weight:800;">데모 모드로 체험하기</a>`;
  }

  // ---------- 🎬 데모 모드: GPS 없이 경로를 따라 자동 이동 (심사·시연용) ----------
  const DEMO = new URLSearchParams(location.search).get('demo') === '1';

  function startDemo() {
    rerouteChecked = true; // 현재 위치 기준 재계산 생략 — 계획된 출발점에서 시작
    const TICK_MS = 400;
    const speeds = [1, 3, 8];
    let demoSpeed = 1;
    ...
    pill.style.cssText = 'top:184px; z-index:600; cursor:pointer; pointer-events:auto;';
    pill.id = 'demoPill';
    document.querySelector('.map-shell').appendChild(pill);
    const renderPill = () => {
      pill.textContent = `데모 체험 중 · ${demoSpeed}배속 (탭하면 배속 변경)`;
    };
    pill.addEventListener('click', () => {
      demoSpeed = speeds[(speeds.indexOf(demoSpeed) + 1) % speeds.length];
      renderPill();
    });
    renderPill();
    ...
    function rebuild() {
      ...
      for (let i = 1; i < pts.length; i++) {
        cum.push(cum[i - 1] + distanceMeters(pts[i - 1][0], pts[i - 1][1], pts[i][0], pts[i][1]));
      }
    }
  ...
  if (window.__MT_DEBUG__) {
    window.__mt = {
      ...
      addLine(pts, opts) {
        const line = makeLine(opts || { color: '#ff9800', weight: 4, opacity: .8, dash: true });
        line.setLatLngs(pts);
        return line;
      },
