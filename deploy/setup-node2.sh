#!/usr/bin/env bash
# node2(10.8.2.5)에서 실행 — 백엔드(9090)를 설치·기동한다.
#
...
if ss -ltn "sport = :$BACKEND_PORT" 2>/dev/null | grep -q LISTEN; then
  echo "  !! $BACKEND_PORT 포트가 이미 사용 중입니다. 스크립트 상단에서 포트를 바꿔 주세요."
  ss -ltnp "sport = :$BACKEND_PORT" 2>/dev/null | tail -n +2
  exit 1
fi
echo "     $BACKEND_PORT 사용 가능"
...
echo "[5/6] 서비스 등록 ..."
cp "$SRC/trevit-backend.service" /etc/systemd/system/
systemctl daemon-reload
systemctl enable --now trevit-backend
systemctl restart trevit-backend
